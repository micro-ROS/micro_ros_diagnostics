  ...
  diagnostic_updater_t * updater)
{
  ...
  diag_msg.name.size = snprintf(diag_msg.name.data, diag_msg.name.capacity, "%s", updater->name);

  // messag
  diag_msg.message.data = malloc(STRSIZE);
  diag_msg.message.capacity = STRSIZE;
  diag_msg.message.size = snprintf(diag_msg.message.data, diag_msg.message.capacity, "%s", updater->message);

  // hardware id
  diag_msg.hardware_id.data = malloc(STRSIZE);
  diag_msg.hardware_id.capacity = STRSIZE;
  diag_msg.hardware_id.size = snprintf(diag_msg.hardware_id.data, diag_msg.hardware_id.capacity, "%s", updater->hardware_id);
  ...
  for (int i = 0; i < updater->num_tasks; ++i) {
    ...
    diag_msg.key.size = snprintf(diag_msg.key.data, diag_msg.key.capacity, "%s", updater->tasks[i]->name);

    diag_msg.value.data = malloc(STRSIZE);
    diag_msg.value.capacity = STRSIZE;
    diag_msg.value.size = snprintf(diag_msg.value.data, diag_msg.value.capacity, "%s", rclc_diagnostic_call_task(updater->tasks[i]));
